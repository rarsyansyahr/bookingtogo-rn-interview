...
const AddVisitorsPage: FC = () => {
  // * Form
  const { onAddVisitor, onRemoveVisitor, control, handleSubmit, onSaveVisitors, fields } =
    useAddVisitorForm();
  ...
  // * Components
  const ListHeader = (
    <Text color={Colors.primary} style={styles.visitorTitle} variant="titleMedium">
      Data Tamu
    </Text>
  );
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <SelectDropdown
            ...
            renderDropdownIcon={(isOpen) => (
              <Icon name={isOpen ? 'menu-up' : 'menu-down'} size={24} />
            )}
          />
        )}
